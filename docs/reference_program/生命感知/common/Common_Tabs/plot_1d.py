...
class Plot1D():
    def __init__(self):
        ...
        self.occThreshPlot.setLabel('bottom', tr('Frames'))
        self.occThreshPlot.setLabel('left', tr('Occupancy Signals'))
        ...
        self.occFrameLen = list(range(1,51))
        self.occPlotIndex = 0
    ...
    def update1DGraph(self, outputDict):
        ...
        if ('occPlot' in outputDict):
            ...
            for box in range(1,len(outputDict['occPlot'])): # skip plotting the first one
                ...
                self.OccData[box].setData(self.occFrameLen,data)
...
#-----------------------------------------------------
    ...
    def setRangeValues(self):
        ...
        if (self.rangeProfileType == 1):
            self.rangePlot.getPlotItem().setLabel('top', tr('Major Range Profile'))
        elif (self.rangeProfileType == 2):
            self.rangePlot.getPlotItem().setLabel('top', tr('Minor Range Profile'))
        else:
            self.rangePlot.getPlotItem().setLabel('top', tr('Range Profile DISABLED'))
