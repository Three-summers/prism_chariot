...
class VideoDoorbell():
    ...
    def initDetDistancesTable(self):
        ...
        self.firstPassDetStats = QTableWidgetItem(tr('Mode 1 Detection Range :'))
        self.secondPassDetStats = QTableWidgetItem(tr('Mode 2 Detection Range :'))
        self.thirdPassDetStats = QTableWidgetItem(tr('Mode 3 Detection Range :'))

        self.detRangeTableItem = QTableWidgetItem(tr('Range (m)'))
        self.detAngleTableItem = QTableWidgetItem(tr('Angle (degrees)'))
        self.detXTableItem = QTableWidgetItem(tr('X Position (m)'))
        self.detYTableItem = QTableWidgetItem(tr('Y Position (m)'))
        self.frameNumTableItem = QTableWidgetItem(tr('Frame Num'))


        self.detRangeTable.horizontalHeader().setSectionResizeMode(QHeaderView.ResizeMode.Stretch)
        self.detRangeTable.verticalHeader().setSectionResizeMode(QHeaderView.ResizeMode.Stretch)
    ...
    def setupGUI(self, gridLayout, demoTabs, device):
        ...
        demoTabs.addTab(self.detDistanceTab, tr('Detection Range'))
        demoTabs.addTab(self.power_report.powerReportTab, tr('Power Consumption Report'))
        demoTabs.addTab(self.false_alarm.falseAlarmTab, tr('False Alarm Report'))
    ...
    def updateGraph(self, outputDict):
        ...
        if ('enhancedPresenceDet' in outputDict):
            if(self.modeSwitchLabel.text() == 'Second Pass Mode' and 2 in outputDict['enhancedPresenceDet']):
                self.modeSwitchLabel.setText(tr('Camera On'))
        ...
        if("modeState" in outputDict):
            if(outputDict['modeState'] == 0): # First pass mode
                self.modeSwitchLabel.setText(tr('First Pass Mode'))
                self.modeSwitchLabel.setFont(QFont('Arial', 16))
                ...
                self.modeSwitchLabel.setText(tr('Second Pass Mode'))
                self.modeSwitchLabel.setFont(QFont('Arial', 16))
                ...
                self.modeSwitchLabel.setText(tr('Third Pass Mode'))
                self.modeSwitchLabel.setFont(QFont('Arial', 16))
        ...
        if (self.tabs.currentWidget() == self.detDistanceTab):
            ...
            if (self.plot_3d.plotComplete):
                self.plotStart = int(round(time.time()*1000))
                self.plot_3d_thread = updateQTTargetThread3D(self.cumulativeCloud, tracks, self.plot_3d.scatter, self.plot_3d, 0, self.plot_3d.ellipsoids, "", colorGradient=self.colorGradient, pointColorMode=self.pointColorMode.currentData(), trackColorMap=self.trackColorMap, clusterLocs=self.clusterLocs)
    ...
    def initStatsPane(self):
        ...
        self.avgPower = QLabel(tr('Measured Power: Calculating...'))
    ...
    def initModeSwitchPane(self):
        modeSwitchBox = QGroupBox(tr('Mode Switch Status'))
        self.modeSwitchLabel = QLabel(tr('Two Pass Mode Disabled'))
        self.modeSwitchLabel.setFont(QFont('Arial', 16))
        self.modeSwitchLabel.setStyleSheet("background-color: lightgrey; border: 1px solid black;")
        self.clearDetectionStatsTable = QPushButton(tr("Clear Detection Table"))
