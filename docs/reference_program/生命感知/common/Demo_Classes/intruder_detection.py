...
###################################################################################################
# The below is a skeleton of what a demo should look like for the Python Visualizer
#
# To add a new demo
# -----------------
# 1) Add it in demo_defines.py in BUSINESS_DEMOS and DEVICE_DEMO_DICT
# 2) Import it into gui_core.py at the top
# 3) Add it to demoClassDict in the Core class
# 4) De
###################################################################### code should be 95-100% self-contained in this file otherwise#############################
...
class IntruderDetection(Plot1D):
    def __init__(self):
        ...
        self.numBoxes = 0   
        self.colors = ['r', 'g', 'b', 'c', 'y', 'm', 'w', 'darkRed', 'darkGreen', 'darkBlue', 'darkCyan', 'darkYellow', 'darkMagenta', 'darkGray']
    ...
    def setupGUI(self, gridLayout, demoTabs, device):
        ...
        demoTabs.addTab(self.myTab, tr('Intruder Detection'))
    ...
    def initStatsPane(self):
        ...
        self.procTimeData = QLabel(tr('Processing Time: {} ms').format(0))

        self.statsLayout = QVBoxLayout()
        self.statsLayout.addWidget(self.frameNumDisplay)
        self.statsLayout.addWidget(self.plotTimeDisplay)
        self.statsLayout.addWidget(self.procTimeData)
    ...
    def updateGraph(self, outputDict):
        ...
        for x in range(1,self.numBoxes):
            if outputDict['occBoxDec'][x] == 1:
                self.occBoxes[x].setBrush(pg.mkBrush((255,0,0)))
    ...
    def parseOccCfg(self, args):
        ...
        label.setFont(QFont("Arial", pointSize=20*scaleFactor))
        label.setZValue(100) # this will put the label on top of the box
