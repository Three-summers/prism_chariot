...
from gui_common import NUM_CLASSES_IN_CLASSIFIER, TAG_HISTORY_LEN, CLASSIFIER_CONFIDENCE_SCORE, MIN_CLASSIFICATION_VELOCITY, TAG_HISTORY_LEN, MAX_NUM_UNKNOWN_TAGS_FOR_HUMAN_DETECTION, resource_path
...
recImgPath = resource_path("images/recImg.jpg")
notRecImgPath = resource_path("images/notRecImg.jpg")
...
class Dashcam(PeopleTracking):
    ...
    def setupGUI(self, gridLayout, demoTabs, device):
        ...
        demoTabs.addTab(self.plot_3d, tr('3D Plot'))
    ...
    def initDashcamGUI(self):
        ...
        cameraBox = QGroupBox(tr("Video Camera Status"))
        self.cameraLayout = QVBoxLayout()
        # set to picture of camera not recording
        self.cameraStatusDisplay = QLabel(tr("camera off"))
        ...
        self.cameraStatusDisplay.setAlignment(Qt.AlignmentFlag.AlignCenter)
        self.cameraStatusDisplay.setPixmap(QPixmap(notRecImgPath))

        self.cameraLayout = QGridLayout()
        self.cameraLayout.addWidget(QLabel(tr("Camera Timeout (seconds)")), 0, 0)
        self.cameraLayout.addWidget(self.cameraTimeoutValueBox, 0, 1)
        self.cameraLayout.addWidget(self.cameraStatusDisplay, 1, 0, 2, Qt.AlignmentFlag.AlignHCenter)
        ...
        if "trackData" in outputDict:
            ...
            for trackNum, trackData in enumerate(tracks):
                ...
                if "camDataDict" in outputDict:
                    ...
                    """
                    self.classifierStr[trackID].setText(tr('Track {}: Distance: {}').format(trackID, trackData[2]))
                    ...
                    self.classifierStr[trackID].setText(tr('Track {}: buf {}: yval {}').format(trackID, str(sum(self.trackDirection[trackID])), trackData[5]))
                    self.classifierStr[trackID].setX(trackData[1])
                    self.classifierStr[trackID].setY(trackData[2] + 0.5)
                    self.classifierStr[trackID].setZ(trackData[3] + 0.1) # Add 0.1 so it doesn't interfere with height text if enabled
