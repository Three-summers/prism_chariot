...
class SurfaceClassification():
    ...
    def setupGUI(self, gridLayout, demoTabs, device):
        ...
        self.surfaceOutput = QLabel(tr("<b>Grass Classification</b><br>") + tr(str(self.surfaceList[0])))
        self.surfaceOutput.setAlignment(Qt.AlignmentFlag.AlignCenter)
        self.surfaceOutput.setStyleSheet('background-color: #46484f; color: white; font-size: 40px; font-weight: light')
        
        self.surfaceOutputRaw = QLabel(tr("<b>Grass Probability</b><br>") + "0.0%")
        self.surfaceOutputRaw.setAlignment(Qt.AlignmentFlag.AlignCenter)
        ...
        self.surfaceDesc.setAlignment(Qt.AlignmentFlag.AlignLeft)
        ...
        self.surfaceOutputRange.getPlotItem().setLabel('bottom', tr('<p style="font-size: 20px;color: white">Relative Frame # (0 is most recent)</p>'))
        self.surfaceOutputRange.getPlotItem().setLabel('left', tr('<p style="font-size: 20px;color: white">Grass Probability Value</p>'))
        self.surfaceOutputRange.getPlotItem().setLabel('right', ' ')
        self.surfaceOutputRange.getPlotItem().setTitle(tr('<p style="font-size: 30px;color: white">Probability Value over Time</p>'))
        ...
        demoTabs.addTab(self.surfaceTab, tr('Surface Classification'))
        demoTabs.setCurrentIndex(1)

    def initSurfacePhysicalSetupPane(self):
        self.surfaceSetupBox = QGroupBox(tr('Physical Setup'))

        self.gestureSetupGrid = QGridLayout()
        self.gestureSetupImg = QPixmap(resource_path('images/surface_setup.png'))
    ...
    def updateGraph(self, outputDict):
        ...
        if ('surfaceClassificationOutput' in outputDict) :
            ...
            if (currentClassification > 0.5) :
                self.surfaceOutput.setText(tr("<b>Grass Classification</b><br>") + tr(str(self.surfaceList[1])))
                ...
                self.surfaceOutput.setText(tr("<b>Grass Classification</b><br>") + tr(str(self.surfaceList[0])))
                self.surfaceOutput.setStyleSheet('background-color: #46484f; color: white; font-size: 40px; font-weight: light')
                self.surfaceOutputRaw.setStyleSheet('background-color: #46484f; color: white; font-size: 40px; font-weight: light')
            
            self.surfaceOutputRaw.setText(tr("<b>Grass Classification Value</b><br>") + "{:8.5f}".format(classification * 100) + "%")
