...
class VitalSigns(PeopleTracking):
    ...
    def initVitalsPlots(self):
        self.vitalsPane = QGroupBox(tr('Vital Signs'))
        ...
        for i in range(MAX_VITALS_PATIENTS):
            ...
            statusLabel = QLabel(tr('Patient Status:'))
            breathLabel = QLabel(tr('Breath Rate:'))
            heartLabel = QLabel(tr('Heart Rate:'))
            rangeBinLabel = QLabel(tr('Range Bin:'))
            ...
            patientDict['breathRate'] = QLabel(tr('Undefined'))
            patientDict['heartRate'] = QLabel(tr('Undefined'))
            patientDict['status'] = QLabel(tr('Undefined'))
            patientDict['rangeBin'] = QLabel(tr('Undefined'))
            ...
            patientPaneLayout.addWidget(statusLabel,0,0,alignment=Qt.AlignmentFlag.AlignHCenter)
            patientPaneLayout.addWidget(patientDict['status'],1,0,alignment=Qt.AlignmentFlag.AlignHCenter)
            patientPaneLayout.addWidget(breathLabel,0,1,alignment=Qt.AlignmentFlag.AlignHCenter)
            patientPaneLayout.addWidget(patientDict['breathRate'],1,1,alignment=Qt.AlignmentFlag.AlignHCenter)
            patientPaneLayout.addWidget(heartLabel,0,2,alignment=Qt.AlignmentFlag.AlignHCenter)
            patientPaneLayout.addWidget(patientDict['heartRate'],1,2,alignment=Qt.AlignmentFlag.AlignHCenter)
            patientPaneLayout.addWidget(rangeBinLabel,0,3,alignment=Qt.AlignmentFlag.AlignHCenter)
            patientPaneLayout.addWidget(patientDict['rangeBin'],1,3,alignment=Qt.AlignmentFlag.AlignHCenter)
        ...
        if (self.vitalsDict is not None and self.numTracks is not None):
            ...
            if (patientId < self.maxTracks):
                ...
                if(float(self.vitalsDict['breathDeviation']) == 0 or self.numTracks == 0):
                    patientStatus = tr('No Patient Detected')
                    breathRateText = tr("N/A")
                    heartRateText = tr("N/A")
                    ...
                    if (float(self.vitalsDict['breathDeviation']) >= 0.02):
                        patientStatus = tr('Presence')
                        if(self.vitalsPatientData[patientId]['breathRate'] == 0):
                            breathRateText = tr("Updating")
                        else:
                     ...
                     # Patient holding breath
                        ...
                        patientStatus = tr('Holding Breath')
                        breathRateText = tr("N/A")
