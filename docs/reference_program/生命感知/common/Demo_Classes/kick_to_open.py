...
class KickToOpen(Plot2D):
    ...
    def setupGUI(self, gridLayout, demoTabs, device):
        ...
        powerPane = QGroupBox(tr("Power Plot"))
        ...
        ktoTempDict['powerplot'].setTitle(tr('Power Usage (mW)'))
        ktoTempDict['powerplot'].setFixedHeight(220)

        # # Set up basic labels so we can edit their appearance
        powerLabel = QLabel(tr('Average Power Usage:'))

        # # Set up data fields
        ktoTempDict['powerUsage'] = QLabel(tr('--.-- mW'))
        ktoTempDict['powerNote'] = QLabel(tr('Please allow 15 seconds after mode switch for power to settle'))
        ...
        powerPaneLayout.addWidget(powerLabel, 1,0,alignment=Qt.AlignmentFlag.AlignHCenter)
        powerPaneLayout.addWidget(ktoTempDict['powerUsage'], 2,0,alignment=Qt.AlignmentFlag.AlignCenter)
        powerPaneLayout.addWidget(ktoTempDict['powerNote'], 3,0, alignment=Qt.AlignmentFlag.AlignCenter)
        ...
        ktoStatusPane = QGroupBox(tr("Status"))
        ktoStatusPaneLayout = QVBoxLayout()
       
        # Setup gesture status box
        gestureBox = QVBoxLayout()
        ktoTempDict['gestureStatus'] = QLabel(tr(self.gestureList[0]))
        ktoTempDict['gestureStatus'].setAlignment(Qt.AlignmentFlag.AlignCenter)
        ...
        modeBox = QVBoxLayout()
        ktoTempDict['modeStatus'] = QLabel(tr("Undefined"))
        ktoTempDict['modeStatus'].setAlignment(Qt.AlignmentFlag.AlignCenter)
        ...
        ktoPlotPane = QGroupBox(tr("Presence Plot"))
        ktoPlotPaneLayout = QGridLayout()


        ktoTempDict['presenceNote'] = QLabel(tr('Searching for presence between 0.25 and 2.25 m'))
        ...
        ktoTempDict['presenceplot'].setTitle(tr('Presence Threshold'))
        ...
        ktoTempDict['dopplerplot'].setTitle(tr('Doppler Average'))
        ktoTempDict['dopplerplot'].setFixedHeight(220)

        # Put the widgets into the layout
        ktoPlotPaneLayout.addWidget(ktoTempDict['presenceNote'], 0, 0,1,1, alignment=Qt.AlignmentFlag.AlignCenter)
        ...
        self.ktoPaneLayout.addWidget(powerPane, 2,0,1,2)
        ...
        # 只创建一次，避免语言切换/重复 setupGUI 时泄漏 running timer
        if not hasattr(self, 'ktoGestureTimer'):
        ...
        demoTabs.addTab(self.ktoPane, tr('Kick to Open'))
    ...
    def updateGraph(self, outputDict):
        ...
        if all(key in outputDict for key in ['ktoGesture', 'gestureFeatures', 'localization_mag', 'localization_range', 'localization_elevidx', 'localization_azimidx']):
            ...
            self.kto['modeStatus'].setText(tr("Gesture Mode"))
        ...
        if ('gesturePresence' in outputDict):
            ...
            if(self.gestureMode != self.gesturePresence):
                if(self.gesturePresence==KTO_PRESENCE_MODE):
                    ...
                    self.kto['modeStatus'].setText(tr("Low Power Mode"))
    ...
    def updateKTOGestureDisplay(self, text):
        ...
        self.kto['gestureStatus'].setText(tr(text))
    ...
    def ktoPowerDataHandler(self, powerData):
        ...
        self.kto['powerUsage'].setText(tr('{} mW').format(powerStr[:5]))
        # print(powerStr)
    ...
    def presenceThresholdHandler(self, presenceThreshold):
        ref = self.presenceDetectCfg
        refLine = pg.InfiniteLine(pos=ref, angle=0, pen='r', label=tr('Presence Threshold Value'))
    ...
    def resetKTOGestureDisplay(self):
        ...
        if(self.gestureMode == KTO_PRESENCE_MODE):
            self.kto['gestureStatus'].setStyleSheet(f'background-color: black; color: white; font-size: 60px; font-weight: bold')
            self.kto['gestureStatus'].setText(tr('Searching for Presence'))
        # gesture mode
        elif(self.gestureMode == KTO_GESTURE_MODE):
            self.kto['gestureStatus'].setStyleSheet(f'background-color: black; color: white; font-size: 60px; font-weight: bold')
            self.kto['gestureStatus'].setText(tr(self.gestureList[0]))
