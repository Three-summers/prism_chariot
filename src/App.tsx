...
import {
  Activity, Clock3, Crosshair, FileText, HeartPulse, Map, Menu, MoreHorizontal, Radio,
  ScanLine, Thermometer, Zap,
} from 'lucide-react'
...
function App() {
  ...
  return <div className="app-shell">
    ...
    {active !== '生命感知' ? <main className="placeholder"><div className="placeholder-icon"><Radio size={42} /></div><h1>{active}</h1><p>功能模块正在建设中，当前页面先展示生命感知驾驶舱。</p><button onClick={() => setActive('生命感知')}>返回生命感知</button></main> : <main className="dashboard">
      ...
      <section className="center-column"><div className="live-heading"><span>实时画面</span><b>|</b><span>车辆：GL-01</span><span className="live-meta">速度：0.38 m/s　|　方向：前进　|　模式：自动巡检</span><span className="online"><i />在线</span></div><div className="scene"><div className="scene-placeholder"><div className="placeholder-grid" /><div className="camera-ring"><span>LIVE</span></div><strong>实时视频占位画面</strong><small>GL-01 · B 区生产线 · 画面源待接入</small></div><div className="scene-wash" /><div className="scene-caption"><span>实时画面</span><b>|</b><span>{now}</span><small>车辆：GL-01　楼层：1F　点位：B1-086</small></div><div className="person-box"><span>人员倒地</span><small>置信度：98%</small></div><div className="vital-callout"><div><span>毫米波生命体征检测（77GHz）</span><b>◉</b></div><p>呼吸频率 <strong>10</strong> <small>bpm</small></p><p>心跳频率 <strong>48</strong> <small>bpm</small></p></div><div className="scan-corner tl" /><div className="scan-corner br" /></div><div className="scene-footer"><span>点位：<b>B1-086</b></span><span>楼层：<b>1F</b></span><span>速度：<b>0.38 m/s</b></span><span>方向：<b>↑ 前进</b></span></div><Panel title="CASE记录" icon={<FileText size={15} />} className="case-panel"><div className="case-table"><div className="case-head"><span>编号</span><span>等级</span><span>时间</span><span>点位</span><span>事件类型</span><span>状态</span><span>响应人</span><span>更新时间</span></div>{cases.map((item, index) => <button className={`case-row ${selectedCase.id === item.id ? 'selected' : ''}`} key={item.time} onClick={() => setSelectedCase(item)}><span>{item.id}</span><span><i className={`level-dot ${item.color}`} />{item.level}</span><span>{item.time}</span><span>{item.spot}</span><span>{item.type}</span><span className={item.state === '处理中' ? 'processing' : 'closed'}>{item.state}</span><span>{item.owner}</span><span>{item.updated}</span></button>)}</div><div className="table-foot">共 5 条记录 <span>‹　1 / 1　›</span></div></Panel></section>
      <aside className="right-column"><Panel title="状态信息" icon={<Activity size={15} />} className="status-panel"><div className="metrics-grid"><Metric label="前方距离" value="12.6" unit="m" /><Metric label="运行速度" value="0.38" unit="m/s" /><Metric label="所在楼层" value="1F" /><Metric label="当前点位" value="B1-086" /><Metric label="呼吸频率" value="10" unit="bpm" /><Metric label="心率" value="48" unit="bpm" /><Metric label="生命状态" value="异常" tone="warning" /><Metric label="事件等级" value="红色" tone="danger" /></div><div className="subheading">生命体征趋势（最近 5 分钟）</div><TrendChart /><div className="system-status"><span><i className="status-dot green" />毫米波雷达</span><b>正常</b><span><i className="status-dot green" />视频流</span><b>实时</b><span><i className="status-dot green" />数据同步</span><b>刚刚</b></div></Panel><Panel title="结案处理" icon={<FileText size={15} />} className="resolution-panel"><div className="resolution-case"><span className="eyebrow">{selectedCase.id} · <span className="danger-text">{selectedCase.type}</span></span><span className="resolution-meta">点位：{selectedCase.spot}　发生时间：{selectedCase.time}</span></div><label>结论<select defaultValue="救助"><option>人员已救助，生命体征稳定，事件关闭</option><option>误报，继续观察</option></select></label><label>响应记录<textarea defaultValue="10:27 支援人员到达现场；10:29 进行初步检查；10:30 人员意识恢复，生命体征稳定。" /></label><div className="form-row"><label>操作人<input defaultValue="张伟" /></label><label>结案时间<input defaultValue="2026-08-12 10:32:44" /></label></div><div className="evidence"><span>证据</span><div><figure><div className="evidence-placeholder">画面</div><figcaption>事故画面<br />10:25:52</figcaption></figure><figure><div className="mini-chart" /><figcaption>生命体征曲线<br />10:25:52</figcaption></figure><figure><div className="responder" /><figcaption>现场照片<br />10:30:05</figcaption></figure><figure><div className="doc-thumb"><FileText /></div><figcaption>处置记录<br />10:30:18</figcaption></figure></div></div><div className="drawer-actions"><button>取消</button><button className="primary">确认结案</button></div></Panel></aside>
    </main>}
  </div>
}
